...
# Ensure vignette building is successful on Windows
is_win   <- .Platform$OS.type == "windows"
is_check <- nzchar(Sys.getenv("_R_CHECK_PACKAGE_NAME_")) || identical(Sys.getenv("CI"), "true")
cpus <- if (is_win && is_check) 1 else 2
```
...
```{r eval = need("wordcloud"), echo = FALSE, warning = FALSE, fit.height = 7.5, fig.width = 7.5}
# plot a word cloud of one cluster as an example
suppressWarnings({
  wordcloud::wordcloud(words = names(cluster_words[[ 5 ]]), 
                     freq = cluster_words[[ 5 ]], 
