...
List fit_lda_c(List &docs, int &Nk, int &Nd, int &Nv, NumericVector alph, 
               NumericMatrix &beta, IntegerMatrix &theta_counts,
               IntegerMatrix &phi_counts,
               IntegerVector &n_d, IntegerVector &n_z,
               List &z_dn,
               int &iterations, int &burnin,
  ...
  // THIS NEEDS TO BE FIXED TO HANDLE COUNTS FROM R
  // for(d = 0; d < Nd; d++){
