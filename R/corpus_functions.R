...
create_dtm <- function(doc_vec, doc_names = names(doc_vec),
  ...
  if (remove_punctuation) {
    ...
    stopword_vec <- unique(unlist(stringr::str_split(
      string = stopword_vec,
      pattern = "\\s+"
    )))
  }
...
dtm_2_docs <- function(dtm, ...) {
  ...
  if (nrow(dtm) > 3000) {
    ...
    out <- tm_parallel_apply(X = dtm_list, FUN = function(x) {
      textmineR::Dtm2DocsC(dtm = x, vocab = colnames(x))
    }, ...)
  } else {
    out <- textmineR::Dtm2DocsC(dtm = dtm, vocab = colnames(dtm))
  }
